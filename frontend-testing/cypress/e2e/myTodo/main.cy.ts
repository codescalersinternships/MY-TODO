...
describe('myTodo app', () => {
  ...
  context('CRUD on 5 items ', () => {
    it('insert 5 new item to the list and check the length of the list', () => {
      for (let i = 0; i < 5; i++) {
        cy.get('#inputField').wait(500).type(`I want to eat burger ${i + 1}!!`);
    ...
    it.only('check 3 items in the 1st page list', () => {      
      cy.get('#todos3').children().should('have.length', 3);
    }) 
    
    it.only('check 2 items in the 2nd page list', () => {  
      cy.get("#page2").click().wait(200);
